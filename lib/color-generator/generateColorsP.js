...
export const generateColorRange = ({
	...
	let firstColor = calculateColor(
		'ffffff',
		color.slice(1),
		1 - lightPercentage
	);
	let lastColor = calculateColor(color.slice(1), '000000', darkPercentage);
	console.log(firstColor);

	if (saturationMultiply) {
		const firstHSL = hexToHSL('#' + firstColor);
		firstHSL.s =
			firstHSL.s * saturationMultiply > 100
				? 100
				: firstHSL.s * saturationMultiply;
		firstColor = HSLToHex(firstHSL.h, firstHSL.s, firstHSL.l).substring(1);

		const lastHSL = hexToHSL('#' + lastColor);
		lastHSL.s =
			lastHSL.s * saturationMultiply > 100
				? 100
				: lastHSL.s * saturationMultiply;
