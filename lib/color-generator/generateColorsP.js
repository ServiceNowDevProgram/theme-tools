class Color {
	constructor(color, name, id, index, scssVariable) {
		this.hex = color;
		this.rgb = hexToRgb(color);
		this.name = name;
		this.id = id;
		this.index = index;
		this.scssVariable = scssVariable;
	}
}
...
export const generateColorScale = ({
	...
	const out = [...lightColors, color, ...darkColors];
	...
	return out.map((color, i) => {
		...
		const name = `--${id}`;
		const scssVariable = `\$${id}`;
		return new Color(color, name, id, index, scssVariable);
	});
};
...
const getSurfaceNeutral = (hex, isDark) => {
	...
	let baseNeutral = isDark ? neutrals[4].hex : neutrals[18].hex;
...
const getSurfaceDivider = (hex, isDark) => {
	...
	const neutrals = getNeutrals(hex);
	let baseNeutral = isDark ? neutrals[8].hex : neutrals[16].hex;
