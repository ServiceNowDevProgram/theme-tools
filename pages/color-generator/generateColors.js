...
const calculateColor = (color1, color2, ratio) => {
	...
	const r = Math.ceil(
		parseInt(color2.substring(0, 2), 16) * ratio + parseInt(color1.substring(0, 2), 16) * (1 - ratio),
	);
	const g = Math.ceil(
		parseInt(color2.substring(2, 4), 16) * ratio + parseInt(color1.substring(2, 4), 16) * (1 - ratio),
	);
	const b = Math.ceil(
		parseInt(color2.substring(4, 6), 16) * ratio + parseInt(color1.substring(4, 6), 16) * (1 - ratio),
	);
...
const generateColors = (color1, color2, variations) => {
  ...
  const numberOfLoops = 1/percentage - 1;
	...
	for (let i = 1; i <= numberOfLoops; i++) {
		colors = [...colors, '#' + calculateColor(color1.slice(1), color2.slice(1), i * percentage)];
	}
...
const generateColorRange = (
	...
	const firstColor = calculateColor('ffffff', color.slice(1), 1 - lightPercentage);
...
export const getColors= (selectedColorGroup, selectedColorId, colorsJson) => {
  ...
  if (colorsJson.base.colors.neutrals && colorsJson.base.colors.neutrals.baseColor) {
    if (selectedColorId === 'surfaceNeutral' || selectedColorId === 'surfaceDivider') {
      const {
      ...
      const newBase = selectedColorId === 'surfaceNeutral' ? neutralColors[4].hex : neutralColors[8].hex;
