...
const path = [{id: 'colorGenerator', href: '/color-generator', label: 'Color Generator'}];
...
class ColorGenerator extends Component {
  ...
  renderColorGroups = () => {
    ...
    if (colorsJson[selectedColorGroup]) {
      return Object.values(colorsJson[selectedColorGroup].colors).map(group => {
        const { id, label, baseColor } = group;
        const isNeutral = id === 'surfaceNeutral' || id === 'surfaceDivider';
        return (
          <div key={id} className="mb-6">
            <label className="block text-sm leading-5 font-medium text-gray-700">{`${label}  ${isNeutral ? '(generated from neutrals)' : ''}`}</label>
              {!isNeutral ?
                <input
  ...
  render() {
    return (
      <Fragment>
        <PageHeader label="Color Generator" path={path} selectedPath={selectedPath} />
        <Page wide>
          <div className="mb-8 flex justify-between items-center">
            <div>
              {this.renderTabs(Object.values(this.state.colorsJson))}
            </div>
            <button
